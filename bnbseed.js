...
contract BNBSeed {
	...
	function getUserbonuspercent(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
				if(user.deposits[i].plan==3 || user.deposits[i].plan==6){
					daliybonus = daliybonus.add(18*users[msg.sender].deposits[i].amount);
				}else if(user.deposits[i].plan==4 || user.deposits[i].plan==7){
					daliybonus = daliybonus.add(28*users[msg.sender].deposits[i].amount);
				}else if(user.deposits[i].plan==5 || user.deposits[i].plan==8){
					daliybonus = daliybonus.add(38*users[msg.sender].deposits[i].amount);
				}
			}
		}
		return daliybonus;
	...
	function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 yourboomplan, uint256 yourbonuspercent) {
		return(getUserTotalDeposits(userAddress), getUserTotalWithdrawn(userAddress), getUserTotalReferrals(userAddress), getyourboomplan(userAddress), getyourbonuspercent(userAddress));
	}
